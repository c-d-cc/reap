...
（例示の数値です。）重要なのは `imports` の行です。`impact` が知っていることはすべて解決済みの import edge から来るため、解決率が低ければグラフは不完全であり、そのとき空の blast radius は「なし」ではなく **「不明」** を意味します。`./x.js` という specifier を、それを生成する `x.ts` に対応づけられない解決器はここで **ゼロ** を報告します — だからこそ「インデックスは走ったか」から推し量るのではなく、解決率を画面に出しています。
