...
(예시 수치입니다.) 중요한 것은 `imports` 줄입니다. `impact` 가 아는 것은 전부 해석된 import edge 에서 나오므로, 해석률이 낮으면 그래프가 불완전하다는 뜻이고 그때 빈 blast radius 는 "없음"이 아니라 **"알 수 없음"**입니다. `./x.js` specifier 를 그것을 만들어내는 `x.ts` 에 대응시키지 못하는 해석기는 여기서 **0** 을 보고합니다 — 그래서 "인덱싱이 돌았는가"로 추론하지 않고 해석률을 화면에 띄웁니다.
