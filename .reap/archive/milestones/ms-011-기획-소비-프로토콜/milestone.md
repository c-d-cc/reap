...
**`flux-0001`이 이 milestone을 줄였다.** 원래 `gen-0045-plan`의 *"REAP는 기획을 쓰지 않는다"* 위에 소비 프로토콜 넷을 짓는 것이었는데, 사람이 그 전제를 되돌렸다 — REAP가 기획을 쓰고(`flux`, `ms-012`), 그래서 **`make issue`(되돌려 보내기)는 사라졌다.** 자기가 쓰는 기획에 자기가 issue를 낼 이유가 없다.
...
**`ms-005`가 이 자리에 있었다.** 쓰는 일(`author-plan`)까지 담았는데, 쓰는 일은 이제 `flux`(`ms-012`)의 것이라 여기서는 등록·읽기·인용만 한다.
