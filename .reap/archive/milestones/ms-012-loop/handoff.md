...
- 도구: `make flux`·`mark flux`(10개 규칙)·`--plan` 거부·상태 줄 — 테스트 `tests/loop.test.ts`
- skill: `plugin/skills/loop/SKILL.md`. `interview`를 가리키는데 아직 없다(`ms-007`)
- 이 리포의 두 번째 loop(`flux-0002-idea`)는 도구 검증용으로 열고 abort했다. **실제 두 번째 loop는 `ms-007`의 `init`이 될 것이다**
