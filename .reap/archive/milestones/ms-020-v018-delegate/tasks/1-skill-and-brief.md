...
| `plugin/skills/evolve/references/delegate-brief.md` (신규) | 템플릿. gen-0076~0082의 지시문(`.reap/archive/generations/`의 Intent와 이 flux의 수행 방식)에서 공통 규율을 추려 en이 아닌 **한국어**로(ms-021이 en으로 간다) |
...
- "닫지 마라"가 brief에 없으면 subagent가 `mark --closed`를 부른다 — 이 flux에서 그것을 매번 명시했다
