...
type: exec
backlog: bk-8adfd0
title: milestone 과잉 분할을 제품에서 막는다 — evolve·carve-milestone·flux 본문, doctor 참고
...
- flux: 닫을 때 산출물이 세대 하나짜리면 backlog 항목(`--from <flux-id>`)으로, milestone 없이 닫는다
...
- flux: 닫기 표에 "세대 하나짜리 산출물 → backlog 항목(--from)", milestone 없이 닫는 경우의 Outcome 기록
