...
끝나는 조건: `grep -rn '@next'`가 리포(archive·inherited·flux 기록 제외)에서 0건, release.yml에 `--tag next` 없음, 게이트(check-docs-surface·check-release-version·hook.test·check-docs-prerender) 통과. 사이트 문구는 ms-022 검수 중인 `ko.ts`와 같은 파일이라 바꾼 자리를 보고에 명시한다.
