...
`ms-007` 7.2 `init` skill + 7.3의 나머지(`init`→`interview`·`carve-milestone`·`flux`) + 씨앗 판정(7.4).

## Outcome

- **`reap init --check`** — 씨앗 그대로인 지식 파일을 번들 템플릿과 비교해 **보고만** 한다. `ms-007`의 Open Question(씨앗 판정을 CLI가 하는가)에 답했다 — `doctor`를 기다리지 않았다. 테스트 둘
- **`plugin/skills/init/SKILL.md`** — 진입 조건 셋(`--check`로 가른다) → 첫 flux를 연다 → 탐색(여섯, 소스 파일 제외, `CLAUDE.md`는 읽되 보고만) → 질문지(plan source → summary → application → evolution 순, 각 물음에 "코드가 답하나"와 쓰는 자리) → `interview` → `carve-milestone` → `--check`·`ctx`로 확인. **묻는 법 문장 0줄**
