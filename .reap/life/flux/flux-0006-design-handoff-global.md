...
- **절을 세대 id로 가르는 안.** agent가 먼저 떠올린 것이다 — 세대 id는 유일하고 영구하며
  나중에 돌이킬 수 있어서 가짜 세션 id 함정을 피한다. 접은 이유는 둘이다. 사람이 세션으로
  지시했고, probe가 진짜 세션 id를 찾아 그 지시가 성립하게 됐다. 세대 id는 **제목에 함께
  싣는 것**으로 남겼다 — 절의 주인을 나중에 알아볼 수 있어야 한다는 요구는 그대로 살아 있다
- **훅 stdin에서 `session_id`를 꺼내 `.session`에 심는 안.** 훅이 받는 JSON에 그 필드가 있다.
  불필요했다 — 환경변수가 이미 모든 자식 프로세스에 있어서 `reap`가 직접 읽으면 된다.
  훅을 거치면 Codex 쪽 배선이 하나 더 생기고, Codex는 애초에 줄 값이 없다
- **`reap handoff` 같은 새 명령.** 절을 쓰고 지우는 것은 산문 편집이라 skill의 일이다.
  도구가 줘야 하는 것은 agent가 만들 수 없는 사실 하나 — 세션 키 — 뿐이고 `ctx`가 이미
  상태 줄을 낸다. 명령을 늘리면 `make`·`mark` 둘로 버틴 표면이 셋이 된다


## Outcome

규범을 `ps-4f2a91`에 썼다(51a4b0c). `03-storage.md`의 **인계는 milestone이 아니라 세션의
것이다** 절이 정본이고 `02-flow.md`·`04-commands.md`·`05-knowledge.md`·`06-agent.md`가
그에 맞춰졌다. 세션 식별 사다리에는 `CLAUDE_CODE_SESSION_ID`가 둘째 칸으로 들어갔다.

milestone을 자르지 않았다 — 실물로 만드는 일이 세대 하나 분량이다. `bk-d7ecf3`가 그
목록을 갖는다.
