...
src/
  ...
  store.ts      .reap/ 레이아웃 — 루트 탐색, 경로, config, 세션, workspace-id, 판(v017·v018·mixed) 판정.
...
- **v0.17 저장소 위에서는 쓰지 않는다.** `store.ts`의 `detectLayout`이 표식으로 판을 가르고,
  `ctx`는 상태 줄 대신 이주 안내만 내며 `doctor`는 결함으로 보고하고 `make`·`mark`는 멈춘다.
  읽기는 막지 않는다 — 막으면 무엇이 잘못됐는지도 못 듣는다. `migrate` skill의
  `detect-version.sh`가 같은 판정을 하므로 **표식 목록은 두 곳을 함께 고친다**
