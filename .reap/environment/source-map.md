...
│   ├── evaluator-run.ts        — **독립 검토가 실제로 있었는가의 단일 소유자** (gen-100). `EvaluatorRun` append + `hasEvaluatorRun` + `recordEvaluatorSilenceIfUnreported` + `formatEvaluatorRun` + `hasUnreviewedStage`. outcome 넷 — `clean`/`concern`/`unreachable`/`not-reported`. `evaluatorConcerns` 만으로는 "검토받고 clean"과 "검토를 못 받았다"가 **바이트 동일**했다. `roundStartedAt`(비공개)이 해당 stage 의 **마지막** timeline 항목으로 라운드를 경계 짓는다 — fitness 는 `completion` 항목을 본다. **stage 재진입 없는 제자리 재작업은 경계 짓지 못한다**(닫을 수 없다 — 코드가 바뀌었는지 REAP 이 알 방법이 없고, 안다고 가정하면 게이트가 된다)
│   ├── prompt.ts               — subagent prompt 공통 모듈 (loadReapKnowledge, buildBasePrompt, buildStrictSection, memory 로딩, cruise 지시, clarity 주입, strict HARD-GATE). **`buildMilestoneSection(milestones, state, level)` 은 milestone 절 텍스트의 단일 소유자** — subagent prompt·evaluator prompt·동적 컨텍스트 async/sync **네 곳**이 부른다. heading 깊이가 인자인 이유는 동적 컨텍스트가 `#` 를 쓰기 때문. 텍스트를 네 번 쓰면 넷이 어긋난다. `buildEvaluatorPrompt(knowledge, paths, state, { stage })` 는 reap-evaluate 용 dynamic context — **§ Record Your Verdict Yourself 와 HARD-GATE 의 `report-evaluator` 예외를 `src/templates/agents/reap-evaluate.md` 와 공유**하며 표식(carrier)과 `tests/unit/evaluator-self-report-shipped.test.ts` 가 묶는다. 배포 사본을 통째로 지워도 이 런타임 사본이 가려주므로 검사 없이는 안 보였다. Code Intelligence 절은 **무조건** 붙는다 (인덱서가 함께 배포되므로 부재할 수 없다) — 절에 적힌 명령은 `tests/e2e/index-command.test.ts` 가 그대로 실행한다
...
│       │   ├── validation.ts   — 검증 (work → complete). `config.evaluator === true` 시 work prompt 에 "Evaluator Subagent Invocation" 절 + `context.evaluator.{enabled, prompt}`; `false` 면 byte-identical. advisor 모델 — builder 가 verdict 를 갖고 evaluator concern 을 surface 한다. `--phase complete` 은 evaluator 활성인데 이번 라운드에 판정이 없으면 `not-reported` 를 **스스로** 기록하고 message 에 적는다 — **전이는 막지 않는다**
│       │   ├── report-evaluator.ts — `run <validation|completion> --phase report-evaluator` 의 **단일 소유자**. validation.ts·completion.ts 가 위임한다. transition graph **밖**(nonce 없음). severity `high|low` → concern + run, `none` → run `clean`, `unreachable` → run 만. 출력 envelope 의 `command` 는 `s.stage` 로 정한다
