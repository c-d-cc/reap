...
## 정정 — commit 직후 도착한 증거 (2026-08-22)

§ 4-a 의 *"agent → 호출자 방향의 회신만 도달하지 않는다"* 는 **과장이다.**

이 세대가 commit 되고 턴이 닫히자 프로브 둘의 회신(`PROBE-OK`, `WROTE`)이 **한꺼번에
도착했다.** 즉 짧은 회신은 **유실된 것이 아니라 턴 경계까지 지연**된 것이다.
다만 **실제 검토 보고 둘(라운드1·2)은 끝내 내용 없이 idle 알림만 왔다.**

정확한 서술: **회신은 작업 중인 호출자에게 도달하지 않는다.** 짧은 것은 턴이 닫힐 때 오고,
긴 검토 보고는 이번에 오지 않았다.

**이 세대의 결론과 수정은 그대로 유효하다** — builder 는 작업 중에 판정을 받아야 하고,
그 시점에는 오지 않으므로 CLI 자기보고가 여전히 유일한 경로다. 바뀌는 것은 **기제**이고,
그것을 틀리게 적어두면 다음 세대가 그 위에서 판단한다. genome longterm 의
*"기제는 맞았고 그 도달 범위를 지어냈다"* 가 이 세대에도 성립했다.
