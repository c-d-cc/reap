...
```bash
mkdir -p .reap/v15
# v0.15 내용을 .reap/v15/로 이동 (genome, environment, life, lineage, hooks, config.yml)
```
...
### 3.5 Environment 복사
```bash
cp .reap/v15/environment/summary.md .reap/environment/summary.md
cp -r .reap/v15/environment/domain/* .reap/environment/domain/ 2>/dev/null
cp .reap/v15/genome/source-map.md .reap/environment/source-map.md 2>/dev/null
```

### 3.6 Lineage 복사
```bash
cp -r .reap/v15/lineage/* .reap/lineage/ 2>/dev/null
```
...
### 3.7 Backlog 복사
```bash
cp -r .reap/v15/life/backlog/* .reap/life/backlog/ 2>/dev/null
```
...
```json
{
  ...
  "message": "Migration complete. Backup at .reap/v15/"
}
```
...
### 3. .reap/v15가 이미 존재하는 경우
- 에러: "이전 마이그레이션 백업이 이미 존재합니다. 먼저 .reap/v15를 제거하세요."

### 7. 백업 자동 삭제
- migration 이후 2~3 generation이 정상 완료되면, completion commit에서 `.reap/v15/` 존재 확인
- 존재하면 자동 삭제 + "v0.15 백업을 삭제했습니다" 안내
- 정상 완료 판단: generation이 completion commit까지 도달 (abort 아님)
