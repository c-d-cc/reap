...
**13초 만에 실행한다**(Bash 로 파일을 쓰게 해서 확인). gen-099 의 evaluator 는 놀고 있던 게
아니라 내내 돌고 있었다 — 그 세대의 관측은 정확했고 **해석이 틀렸다.**

**정정 (commit 직후 관측)**: 04-validation 은 *"회신 방향만 도달하지 않는다"* 라고 적었는데
**과장이다.** 세대가 끝나고 턴이 닫히자 프로브의 짧은 회신 둘(`PROBE-OK`·`WROTE`)이
**한꺼번에 도착했다** — 유실이 아니라 **턴 경계까지 지연**이다. 다만 **검토 보고 둘은 끝내
내용 없이 idle 알림만 왔다.** 실무적 결론은 그대로다(**기다리는 builder 에게는 안 온다**)
지만 기제는 다르고, 다음 세대는 이 차이 위에서 판단해야 한다.
**genome longterm 의 "기제는 맞고 도달 범위를 지어냈다"가 이 세대에도 성립했다.**
