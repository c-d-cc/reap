...
**0.17.5 에 전부 합친다 (유저 결정 2026-08-19).** 별도 0.17.6 을 내지 않고 아래 8건을 0.17.5 에 포함해 한 번에 릴리즈한다. 따라서 세대 작업이 끝난 뒤 RELEASE_NOTES / NOTICE / 5 로케일을 **추가 내용으로 보강**해야 한다 — 지금 쓰인 것은 daemon 배포·위치조회 범위뿐이다.
...
**추가 8건 (원래 0.17.6 이었던 것) — gen-083/084 작업에서 파생.** 전부 오늘 드러난 결함이며 서로 맞물려 있다.
