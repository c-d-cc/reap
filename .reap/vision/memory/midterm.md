...
**발행 문서 수정 규칙 (2026-08-21 갱신 — 2026-08-20 판단을 대체한다)**:
"이미 나간 문서는 손대지 않는다"가 원칙이되 **예외가 있다.** 예외 기준은 *역사성*이 아니라
**실행 가능성**이다 — 이미 나간 changelog 라도 **따라 하면 실패하는 지시**(예:
`npm i -g <deprecated>`, 존재하지 않는 명령)를 담고 있으면 그 부분은 고친다.
고칠 때도 **버전 항목 자체는 남긴다** — "역사 기록"이 요구하는 것은 항목의 존재이고
새 지시가 요구하는 것은 특정 서술의 부재라, 둘이 겹치지 않는다.
gen-094 가 이 기준으로 v0.17.5·v0.17.0 을 처리했고 사용자가 승인했다.
**이 문단만 읽고 gen-094 의 변경을 되돌리지 말 것.**
...
**backlog pending 15건** (gen-094 가 2건 만들고 2건 다 소비했다). 정리 기준은 2026-08-19 유저 지시 그대로 — **지금 존재하는 사용자에게
