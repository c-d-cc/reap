...
### Agent 정의 파일로 context 절약
subagent prompt에 guide+genome+environment를 직접 넣으면 ~500줄.
`.claude/agents/` 정의 파일에 "파일에서 읽어라" 지시를 넣으면 context 절약 가능.
다만 subagent가 실제로 파일을 읽는지는 보장 안 됨 — 품질 관찰 필요.
