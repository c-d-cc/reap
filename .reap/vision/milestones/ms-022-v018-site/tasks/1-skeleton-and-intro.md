...
- 쪽: `introduction.md`(REAP 한 문장, 원칙 일곱 요약 — spec 01 링크), `install.md`(README 설치 절과 같은 명령 — README를 가리키지 말고 여기 완전하게), `quick-start.md`(init→evolve→complete, 상태 줄 실물 예시 — `reap ctx` 출력을 실제로 찍어 넣는다), `concepts.md`(세 층·flux/milestone/generation·3단 저장소·map)
