...
|---|---|---|---|
| /docs/two-axes | 두 축 | Plan 축(flux가 plan을 개선)과 Execution 축(milestone으로 잘라 generation으로 진행)이 왜 대등한가, 만나는 지점(carve), 각 축의 사이클이 어떻게 따로 도는가(flux는 여러 세션·여럿 병렬, generation은 세션 바인딩·하나) | CoreConceptsPage의 개념 표 형식 |
...
| /docs/flux | Flux | 유형 넷, 열고 잇고 닫기, Question·Dialogue·Dead Ends·Outcome, 닫히는 조건(산출물이 자리를 찾음), 열린 채 두는 것이 정상, `make flux`·`mark flux` | LifecyclePage의 절 구성(단계별 설명) |
