...
- A new `handoff` skill, and you can call it yourself. Ending a session without going through `complete` used to leave nothing behind; now `/reap:handoff` writes, replaces or clears your section. Its first step is deciding whether one is warranted at all: a handoff carries what *this* session did that the next one needs in order to carry on, and nothing else — not an open milestone, not a question still being chewed on
