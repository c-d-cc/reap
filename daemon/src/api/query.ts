...
export function createQueryHandlers(
  ...
  async function withManager(projectId: string, worktree: string | undefined, fn: (mgr: IndexManager) => ApiResponse | Promise<ApiResponse>): Promise<ApiResponse> {
    try {
      const mgr = await getIndexManager(projectId, worktree);
      return await fn(mgr);
    } catch (e) {
  ...
  return {
    async searchSymbols(params, _body, query) {
      return withManager(params.id, query.worktree, (mgr) => ({
        status: "ok", data: mgr.searchSymbols(query.q ?? "", query.type),
      }));
    },
    async getSymbol(params, _body, query) {
      return withManager(params.id, query.worktree, (mgr) => {
    ...
    async getCallers(params, _body, query) {
      return withManager(params.id, query.worktree, (mgr) => {
        const symbolId = decodeURIComponent(params.symbolId);
    ...
    async getCallees(params, _body, query) {
      return withManager(params.id, query.worktree, (mgr) => {
        const symbolId = decodeURIComponent(params.symbolId);
    ...
    async getFileSymbols(params, _body, query) {
      return withManager(params.id, query.worktree, (mgr) => ({
        status: "ok", data: mgr.getFileSymbols(decodeURIComponent(params.path)),
      }));
    },
    async getFileDependencies(params, _body, query) {
      return withManager(params.id, query.worktree, (mgr) => ({
    ...
    async getCommunities(params, _body, query) {
      return withManager(params.id, query.worktree, (mgr) => ({ status: "ok", data: mgr.getCommunities() }));
    },
    async getCommunity(params, _body, query) {
      return withManager(params.id, query.worktree, (mgr) => {
    ...
    async getProcesses(params, _body, query) {
      return withManager(params.id, query.worktree, (mgr) => ({ status: "ok", data: mgr.getProcesses() }));
    },
    async getProcess(params, _body, query) {
      return withManager(params.id, query.worktree, (mgr) => {
