...
- `--back` 단독: 바로 이전 stage로 회귀 (예: validation → implementation)
- `--back [stage]`: 지정한 stage로 직접 회귀 (예: `--back planning`)
- 첫 stage(objective)에서는 회귀 불가

**회귀 프로토콜:**
1. 인간에게 회귀 사유를 물어라
2. 회귀 사유와 참조 정보를 구체적으로 수집하라:
   - **reason**: 왜 돌아가는지 (예: "validation에서 lint 에러 3건 발견")
   - **refs**: 관련 파일/위치 (예: `04-validation.md#발견된-문제`, `src/app.tsx:21`)
3. `current.yml`을 수정하라:
   ...
   - `timeline`에 회귀 entry를 추가:
     ...
     ```
4. 산출물 처리:
   - **대상 stage 이전 산출물**: 보존 (건드리지 않음)
   - **대상 stage 산출물**: 재진입 시 덮어쓰기 (이 stage를 다시 하려고 돌아온 것)
   - **대상 stage 이후 산출물**: 보존하되, 해당 stage 재진입 시 덮어쓰기
5. 회귀 사유를 대상 stage 산출물 상단에 `## Regression` 섹션으로 기록하라:
   ...
   - **Reason**: [회귀 사유]
   - **Refs**: [참조 목록]
   - **Affected**: [영향받는 이후 산출물 목록]
   ```
6. "[stage] 단계로 복귀했습니다. `/reap.[stage]`로 진행하세요." 안내

**예시:**
- validation → implementation 회귀: 02-planning.md 보존, 03-implementation.md append, 04-validation.md 보존 (재진입 시 덮어쓰기)
- validation → planning 회귀: 01-objective.md 보존, 02-planning.md 덮어쓰기, 03/04 보존 (재진입 시 덮어쓰기)
