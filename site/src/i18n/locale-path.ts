 ...
 *
 * `DEFAULT_LOCALE` carries no prefix. `README*.md` links `https://reap.cc/docs/*`
 * directly, and those READMEs ship inside the npm tarball — a prefixed default
 * would break every one of them, in copies already published.
 */
import { LOCALES, DEFAULT_LOCALE, type Locale } from "./types";

/** The path segment that identifies each locale. The default locale has none. */
