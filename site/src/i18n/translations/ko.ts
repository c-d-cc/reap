...
export const ko: Translations = {
  ...
  hero: {
    ...
    problems: [
      { problem: "파이프라인이 흐름을 강제했다", solution: "흐름은 CLI가 아니라 skill이 판단한다 — 통과·차단하는 게이트가 없다" },
      { problem: "작업 단위가 하나였다", solution: "loop·milestone·generation 셋으로 나뉜다 — 기획과 실행이 서로 다른 사이클로 돈다" },
      { problem: "사람 평가가 매 세대마다 필요했다", solution: "milestone이 끝날 때만 사람의 fitness 평가를 받는다" },
      { problem: "기획 산출물이 REAP 소유였다", solution: "리포 안팎 어디든 있을 수 있는 등록된 plan source다" },
    ],
    threeLayer: "세 개의 층",
    threeLayerDesc: "REAP에서 일어나는 모든 일은 판단·확정·사실 중 하나에 속한다. agent는 판단하고, CLI에게 확정을 요청하고, 사실은 git에게 직접 묻는다.",
    layers: [
      { label: "판단", sub: "skill을 읽는 agent", desc: "무엇을 할지, 언제 할지, 됐는지" },
      { label: "확정", sub: "reap CLI", desc: "확률에 맡길 수 없는 사실을 못 박는다 — id 발급, frontmatter, 원자적 선점" },
      { label: "사실", sub: "git, 파일시스템", desc: "실제로 무슨 일이 있었는가 — 커밋 유무, 작업 트리 상태" },
    ],
    installation: "설치",
    installStep1: "1. CLI 전역 설치",
    installStep2: "2. Claude Code에서 초기화하고 첫 세대를 연다",
    installNote: "설치 상세와 Claude Code 플러그인은",
    installLinkText: "설치 가이드",
    keyConcepts: "일곱 원칙 중 셋",
    concepts: [
      { label: "REAP는 흐름을 제어하지 않는다", desc: "세대 내부의 순서도, 언제 열고 닫을지도 skill이 기술하고 agent가 판단한다" },
      { label: "확률에 의존하면 안 되는 것만 스크립트가 소유한다", desc: "id 발급, frontmatter, 세션 바인딩, 원자적 선점. 나머지는 판단이다" },
      { label: "사람이 적합도를 판정한다", desc: "정량 지표는 없다. milestone이 끝날 때 사람의 자연어 피드백이 유일한 신호다" },
    ],
    documentation: "문서",
    docLinks: [
      ...
      { href: "/docs/skills", title: "skill 10종", desc: "agent가 REAP를 다루는 통로. 언제·무엇을·부르지 않는 경우." },
      { href: "/docs/cli", title: "CLI 레퍼런스", desc: "reap 명령 전체 — make·mark·doctor·index·orch·ctx." },
      ...
      { href: "/docs/orchestrate", title: "orchestrate", desc: "두 세션 이상이 동시에 작업할 때 — claim과 barrier." },
      { href: "/docs/migration", title: "v0.17에서 이주", desc: "8단계 이주. 원본은 .reap-v0_17/에 보존된다." },
