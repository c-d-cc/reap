...
**Fitness phase + cruise mode** (gen-067): the evaluator also runs during the fitness phase, and `report-evaluator` is callable from both stages — `reap run validation --phase report-evaluator` and `reap run completion --phase report-evaluator`, with `--severity high|low` (a concern), `none` (reviewed, no concern) or `unreachable` (no verdict arrived). High-severity concerns recorded during validation **automatically abort cruise mode** when the next fitness phase runs — `cruiseCount` is cleared from `config.yml`, the cruise prompt is replaced with a supervised fallback, and the human reviews the concern before composing fitness feedback. Cruise can be resumed manually with `reap cruise <N>` once the concern is resolved. Low-severity concerns surface in the prompt's "Prior Evaluator Concerns" section without aborting cruise.

**Whether the review happened at all** (gen-100): the verdict does not travel only through the evaluator's reply to the builder. The evaluator has `Bash` and records its own verdict with the same CLI, so a reply that never arrives no longer loses the review — which is what happened once, with the subagent running correctly the whole time and only the reply direction lost. And when nothing is reported, REAP writes that down itself at `validation --phase complete` and `completion --phase adapt`: `state.evaluatorRuns` gets a `not-reported` entry, and the fitness prompt tells the human this generation went unreviewed. None of it gates anything — an environment may have no Agent tool at all, and the transition happens either way.
