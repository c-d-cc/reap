...
5.5. **Release Notice 작성 및 게시** (유저 컨펌 필수):
   - RELEASE_NOTES.md의 내용을 기반으로 release notice 초안을 작성한다
   - 분량: 평소 5줄 내외, breaking change가 있으면 상세하게
   - 다국어 섹션 포함:
     ```
     ## en
     ...
     [한국어 릴리스 노트]
     ```
   - 제목 형식: `v{new} Release Notes`
   - 초안을 유저에게 보여주고 컨펌을 받는다
   - 컨펌 후 GitHub Discussions (Announcements 카테고리)에 게시:
     ```bash
     # 1. Repository ID와 Announcements 카테고리 ID 조회
     REPO_ID=$(gh api graphql -f query='{ repository(owner: "c-d-cc", name: "reap") { id } }' -q '.data.repository.id')
     CAT_ID=$(gh api graphql -f query='{ repository(owner: "c-d-cc", name: "reap") { discussionCategories(first: 10) { nodes { id name } } } }' -q '.data.repository.discussionCategories.nodes[] | select(.name == "Announcements") | .id')
     ...
     ' -f repoId="$REPO_ID" -f catId="$CAT_ID" -f title="v{new} Release Notes" -f body="<notice 본문>"
     ```
   - 게시 성공 시 Discussion URL을 유저에게 안내
