...
const TRANSLATIONS: Record<Locale, Translations> = { en, ko, "zh-CN": zhCN, de, ja };
...
export interface PrerenderedPage {
  /** Path relative to the build output root, e.g. `ko/docs/genome/index.html`. */
  filePath: string;
  /** The URL this file will be served at, e.g. `/ko/docs/genome`. */
  href: string;
  locale: Locale;
  route: string;
  html: string;
...
function headFor(route: string, locale: Locale, origin: string): string {
  ...
  lines.push(
    `<link rel="alternate" hreflang="x-default" href="${localeUrl(origin, "en", route)}" />`,
  );
...
export function renderAll(template: string, origin: string): PrerenderedPage[] {
  ...
  for (const locale of LOCALES) {
    for (const def of ROUTES) {
      ...
      // `/` -> `index.html`, `/ko/` -> `ko/index.html`,
      // `/ko/docs/genome` -> `ko/docs/genome/index.html`.
  ...
  const urls = LOCALES.flatMap((locale) =>
    ROUTES.map((def) => {
      const alternates = [
        ...
        `    <xhtml:link rel="alternate" hreflang="x-default" href="${localeUrl(origin, "en", def.path)}" />`,
      ].join("\n");
