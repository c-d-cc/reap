...
export const zhCN: Translations = {
  ...
  concepts: {
    ...
    stages: [
      ...
      ["Implementation", "AI + 人类协作实现", "03-implementation.md"],
      ["Validation", "执行测试、验证完成标准", "04-validation.md"],
      ["Completion", "回顾 + Genome更新 + 压缩", "05-completion.md"],
    ],
...
strict:
  ...
  hooks: {
    ...
    eventItems: [
      ...
      ["onGenomeResolved", "合并Generation中Genome冲突解决之后"],
...
# npm run lint`,
  ...
  mergeCommands: {
    ...
    mergeHookItems: [
      ["onMergeStart", "/reap.merge.start创建合并Generation之后"],
      ["onGenomeResolved", "Genome冲突解决之后（mate → merge转换）"],
      ["onMergeComplete", "合并Generation归档之后"],
    ],
  ...
  environmentPage: { title: "Environment", breadcrumb: "指南", intro: "Environment存储影响项目的外部上下文 — API、基础设施、组织规则和参考材料。", structureTitle: "3层结构", structure: `.reap/environment/\n├── summary.md      # 会话上下文 (~100行，自动加载)\n├── docs/           # 主要参考文档\n└── resources/      # 原始材料 (用户管理)`, layersTitle: "层级", layerHeaders: ["层级", "维护主体", "内容", "限制"], layerItems: [["summary.md", "AI (自动生成)", "docs/全部概述。每次会话加载。", "~100行"], ["docs/", "AI + 用户", "按环境主题的文件。", "每文件~100行"], ["resources/", "用户", "原始文档、PDF、外部链接。", "无限制"]], flowTitle: "查找流程", flowDesc: "summary.md (始终加载) → docs/ (需要详情时) → resources/ (需要原始来源时)", syncTitle: "同步", syncDesc: "使用/reap.sync.environment从源代码发现外部依赖，通过用户访谈进行文档化。" },
  lifecyclePage: { title: "生命周期", breadcrumb: "指南", intro: "生命周期是REAP的心脏 — 每个Generation经历5个阶段（Objective → Planning → Implementation → Validation → Completion），每步生成制品。AI代理引导您完成整个生命周期。" },
