...
commit:  1a2b3c4`,
  ...
  visionPage: {
    ...
    memoryHeaders: ["层级", "作用", "判断标准"],
    memoryRows: [
      ["Shortterm", "会话交接——传递给下一次会话的上下文", "「现在就需要吗？」"],
      ["Midterm", "进行中的轨道——跨多代的工作", "「是尚未完成的大块工作吗？」"],
      ["Longterm", "设计经验——值得日后回读", "「能避免重犯同样的错误吗？」"],
    ],
  ...
  releaseNotes: {
    ...
    versions: [
      {
        version: "0.17.7",
        notes: "**文档站点为每个页面提供的都是同一个空壳。** `reap.cc/docs/*` 对每个地址都返回 HTTP 404，自站点上线以来一直如此 —— 它是 GitHub Pages 上的单页应用，所以每个 URL 拿到的都是同一个 939 字节的壳，壳启动后路由器画出正确的页面，**在人眼里这是一个完美的站点**。搜索引擎看到的是 404，除了根路径什么都没有索引。**现在每个页面都在构建时渲染**：5 种语言 × 23 条路由 = **115 个文件**，各自拥有自己的正文、`<title>`、`<meta description>`、canonical 与 `hreflang`，还新增了此前并不存在的 `sitemap.xml` 和 `robots.txt`。最小的页面从 939 字节变成 15 kB。**每种语言有了自己的地址。** 英文沿用原有地址 —— `reap.cc/docs/quick-start` —— 其余四种带前缀：`/ko/docs/quick-start`、`/ja/...`、`/de/...`、`/zh-CN/...`。此前五种语言共用一个 URL 且由浏览器决定，因此爬虫只能索引其中一种，你分享的链接对不同的人显示不同的语言。首次进入 `/` 时仍会按每个标签页一次的规则跳转到你的语言；其他任何地址都保持你请求的样子。**已停用的 daemon 不只从代码中消失，也从文档中消失了** —— 其中包括一份迁移说明，它指示安装 `npm i -g @c-d-cc/reap-daemon`（已废弃的包），并注入到从未启用过 daemon 的项目的每次会话上下文中。**`/docs/daemon` 现在是 `/docs/code-intelligence`；如果你收藏了旧地址，它将无法打开。** CLI 没有任何变化。",
      },
      {
        version: "0.17.6",
      ...
      {
        version: "0.17.1",
        notes: "**迁移指令层（Migration instruction layer）** — `reap update` 会检测项目的 `lastMigratedVersion` 与已安装 REAP 版本之间的差距，并将对应版本的迁移说明注入代理的 SessionStart 上下文。代理由此获得升级后重组现有产物的具体指令；应用完成后运行 `reap update --mark-migrated`。**Memory 结构改进** — vision memory 改为基于内容类型的层级，并在 reflect 阶段新增强制 pruning 策略。",
      },
      {
        version: "0.17.0",
      ...
      {
        version: "0.16.5",
        notes: "**OpenCode 适配器** — 通过 `agentClient: opencode` 完整支持多客户端。自动管理 `opencode.json` 的 instructions、`.opencode/plugins/reap-plugin.ts`、AGENTS.md 以及 `~/.config/opencode/commands/` 下的斜杠命令。**Early-close 生命周期路径** — 可在 implementation 或 validation 阶段使用的轻量结束方式（`reap run early-close`、`/reap.early-close`）。保留部分成果，执行简化的 reflect，并将未完成的任务自动继承为下一代的 backlog。当用户表达中止或缩减范围的意图时，代理会明确提供 abort / early-close / continue 三个选项。",
      },
      {
        version: "0.16.4",
