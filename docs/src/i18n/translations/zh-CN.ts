...
commit:  1a2b3c4`,
  ...
  releaseNotes: {
    ...
    versions: [
      {
        version: "0.17.8",
        notes: "**v0.18 已在 npm `next` 标签上发布** —— 不会通过自动更新到达。运行 `reap update` 会安装一个引导式升级 agent，指引你安装 v0.18 并迁移项目。**v0.18 默认使用英语** —— 在 `.reap/config.yml` 中设置 `language: ko` 可切换为韩语。版本检查现在最多每天执行一次，会话启动因此更快。",
      },
      {
        version: "0.17.7",
