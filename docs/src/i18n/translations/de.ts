...
commit:  1a2b3c4`,
  ...
  releaseNotes: {
    ...
    versions: [
      ...
      {
        version: "0.17.6",
        notes: "**Ein Code-Index ist eingebaut.** `reap index` genügt: nichts zu installieren, kein Port, kein Hintergrundprozess: `reap index status | search | impact <file> | callers <symbolId> | callees <symbolId>`. Fünfzehn Sprachen, kein nativer Build. Der Index ist an den Commit gebunden, liegt in `.reap/.index/` und wird von git ignoriert, sodass nicht committete Arbeit bewusst nicht enthalten ist.\n\n**`reap uninstall` entfernt, was npm nicht kann.** Slash-Befehle, Agent-Definitionen, `~/.reap/` und die SessionStart-Hook-Einträge überleben `npm uninstall -g`, und der zurückgebliebene Hook rief weiterhin einen Befehl auf, den es nicht mehr gab. Paket schon entfernt? `npx @c-d-cc/reap uninstall --confirm`.\n\n**REAP in ein Projekt zu installieren ändert Ihre globale Installation nicht mehr.** Eine projektlokale Kopie, ein `npx`-Lauf und ein Quell-Checkout lassen Ihre Maschine jetzt in Ruhe.\n\n**`autoUpdate: false` schaltet die automatische Aktualisierung jetzt wirklich ab.** Die Einstellung steht seit v0.16 in `.reap/config.yml` und wurde von nichts gelesen. Sie stoppt die Installation und nur diese — die Warnung, dass Ihre Version zu alt für eine automatische Korrektur ist, erreicht Sie weiterhin.",
      },
      {
        version: "0.17.5",
        notes: "**REAP installiert seine eigene Integration beim ersten Lauf.** npm 12 blockiert Install-Skripte bei globalen Installationen, wodurch Slash-Befehle, Agent-Definitionen und der Session-Hook ohne jede Fehlermeldung nicht abgelegt wurden — die Binärdatei lief, die Integration fehlte schlicht.\n\n**`reap run push` meldet den tatsächlichen Fehler von git** statt einer Vermutung.\n\n**`reap help` führt `/reap.run` und `/reap.report` auf.**",
      },
      {
        version: "0.17.4",
