...
reap daemon query    # Symbol-Abfrage ausführen`,
  ...
  releaseNotes: {
    ...
    versions: [
      {
        version: "0.17.5",
        notes: "**Der Code-Intelligence-Daemon existiert jetzt wirklich** — `daemon: true` war seit v0.16 dokumentiert, aber `@c-d-cc/reap-daemon` wurde nie veröffentlicht: npm-Installationen erhielten einen toten Symlink, und jeder Daemon-Aufruf schlug still fehl. Das Paket ist jetzt auf npm und keine Abhängigkeit von REAP mehr — wer `daemon: true` nutzt, führt einmal `npm i -g @c-d-cc/reap-daemon` aus. **Dahinter lagen drei weitere Defekte** — das Bundle band seine nativen Bindings ein und startete daher nur unter bun, es suchte seine Tree-sitter-Queries ein Verzeichnis zu hoch, sodass die Indizierung null Symbole lieferte, und `__dirname` wurde zur Bauzeit fest eingesetzt, wodurch das veröffentlichte v0.17.4-Bundle auf jeder Maschine auf den Checkout des Maintainers zeigte. **Ein fehlender Daemon ist jetzt sichtbar** — `reap daemon status`, `reap fix --check`, der Agent-Prompt und die Lifecycle-Ausgabe melden es; ein zu alter Daemon wird anders formuliert. **`daemonBin`, wenn beide sich nicht finden** — in `.reap/config.yml` setzen oder `REAP_DAEMON_BIN` für einen einzelnen Aufruf; ein leerer benannter Ort wird gemeldet statt ignoriert, und `daemon status` zeigt `bin` und `binSource`. **`reap help` listet `/reap.run` und `/reap.report`** — bisher fehlten beide.",
      },
      {
        version: "0.17.4",
