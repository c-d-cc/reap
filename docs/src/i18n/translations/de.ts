...
commit:  1a2b3c4`,
  ...
  releaseNotes: {
    ...
    versions: [
      ...
      {
        version: "0.17.6",
        notes: "**`reap index` — ein Code-Index ist eingebaut.** Nichts zu installieren, kein Port, kein Hintergrundprozess: `reap index status | search | impact | callers | callees`. Fünfzehn Sprachen, kein nativer Build. Der Index ist an den Commit gebunden, liegt in `.reap/.index/` und wird von git ignoriert — nicht committete Arbeit ist bewusst nicht enthalten.\n\n**`reap uninstall`** entfernt, was npm nicht kann — Slash-Befehle, Agent-Definitionen, `~/.reap/` und die SessionStart-Hook-Einträge bleiben bei `npm uninstall -g` zurück. Paket schon entfernt? `npx @c-d-cc/reap uninstall --confirm`.\n\n**REAP in ein Projekt zu installieren ändert Ihre globale Installation nicht mehr**, und **`autoUpdate: false` schaltet die automatische Aktualisierung jetzt wirklich ab** — die Einstellung stand seit v0.16 in `.reap/config.yml` und wurde von nichts gelesen.\n\n**Der Daemon ist stillgelegt.** `reap update` entfernt `daemon` / `daemonBin` aus Ihrer Konfiguration und löscht `~/.reap/daemon/`. Das globale Paket entfernen Sie mit `npm uninstall -g @c-d-cc/reap-daemon`.",
      },
      {
        version: "0.17.5",
        notes: "**REAP installiert seine eigene Integration beim ersten Lauf.** npm 12 blockiert Install-Skripte bei globalen Installationen, wodurch Slash-Befehle, Agent-Definitionen und der Session-Hook nicht abgelegt wurden — ohne jede Fehlermeldung. Die Binärdatei lief, die Integration fehlte schlicht. **`reap run push` meldet jetzt den tatsächlichen Fehler von git** statt einer Vermutung, und **`reap help` führt `/reap.run` und `/reap.report`** auf.",
      },
      {
        version: "0.17.4",
