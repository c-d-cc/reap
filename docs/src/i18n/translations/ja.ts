...
export const ja: Translations = {
  ...
  cli: {
    ...
    uninstallNote: "npmはパッケージだけを削除します。REAPのスラッシュコマンド、エージェント定義、SessionStartフック、~/.reap/ はREAP自身のコードが書いたもので、npmはアンインストール時にコードを実行しません。このコマンドが正しい順序で処理します — 両クライアントのユーザーレベルファイルを削除し、settings.jsonからREAPの項目だけを取り除き、REAPが ~/.reap/ に置いたものを削除し、その後 REAP の npm パッケージをnpmに渡します。あなた自身のファイルは残ります — reapdev.* も、~/.reap/ 内のその他も。すでにパッケージを削除した場合は npx @c-d-cc/reap uninstall --confirm。1つのプロジェクトからREAPを取り除く reap destroy とは別物です。",
...
commit:  1a2b3c4`,
  ...
  visionPage: {
    ...
    memoryHeaders: ["階層", "役割", "判断基準"],
    memoryRows: [
      ["Shortterm", "セッション引き継ぎ — 次のセッションに渡す文脈", "「今すぐ必要か？」"],
      ["Midterm", "進行中のトラック — 複数 Generation にまたがる作業", "「まだ完了していない大きなトラックか？」"],
      ["Longterm", "設計上の教訓 — 後で読み返す価値があるもの", "「同じ失敗を防ぐか？」"],
    ],
  ...
  releaseNotes: {
    ...
    versions: [
      {
        version: "0.17.7",
        notes: "**ドキュメントサイトはすべてのページに空のシェルを一つ返していました。** `reap.cc/docs/*` はすべてのアドレスで HTTP 404 を返しており、サイト公開以来ずっとそうでした — GitHub Pages 上の SPA なので、どの URL も同じ 939 バイトのシェルを受け取り、シェルが起動してルーターが正しいページを描くため、**人の目には完全なサイト**に見えていました。検索エンジンは 404 を見て、ルート以外は何も索引しませんでした。**すべてのページがビルド時にレンダリングされるようになりました**: 5 言語 × 23 ルート = **115 ファイル**、それぞれが自分の本文・`<title>`・`<meta description>`・canonical・`hreflang` を持ち、存在しなかった `sitemap.xml` と `robots.txt` も追加されました。最小のページは 939 バイトから 15 kB になりました。**言語ごとにアドレスができました。** 英語はこれまでのアドレスをそのまま使い（`reap.cc/docs/quick-start`）、他の四つには接頭辞が付きます: `/ko/docs/quick-start`、`/ja/...`、`/de/...`、`/zh-CN/...`。以前は五つの言語が一つの URL を共有しブラウザが選んでいたため、クローラーはそのうち一つしか索引できず、共有したリンクは開く人ごとに違う言語を表示していました。`/` に最初に来たときはタブごとに一度だけ自分の言語へ移動し、それ以外のアドレスは要求されたまま残します。**廃止されたデーモンがコードだけでなくドキュメントからも消えました** — その中には、`npm i -g @c-d-cc/reap-daemon`（非推奨パッケージ）のインストールを指示し、デーモンを有効にしたことのないプロジェクトの毎セッションのコンテキストに注入されていた移行ノートもありました。**`/docs/daemon` は `/docs/code-intelligence` になりました。古いアドレスをブックマークしている場合は開けなくなります。** CLI に変更はありません。",
      },
      {
        version: "0.17.6",
        notes: "**REAP にコードインデックスが内蔵されました。** インストールするものもポートも常駐プロセスもありません — `reap index status`、`reap index impact <file>`、`reap index search <query>`、`reap index callers/callees <symbolId>`。15 言語対応、ネイティブビルドなし。**blast radius は標準的な TypeScript プロジェクトで常にゼロを返していました** — 5 か月間。resolver が `./x.js` という specifier を、それを生成する `x.ts` に対応づけられなかったためです。テスト 130 件が通り CI もずっと緑でした。どの検査も「インデックスは走ったか」を問い、「その答えは意味をなすか」を誰も問わなかったからです。**そこで `reap index status` が import 解決率を報告するようになり**、リリースゲートは「シンボル数 > 0」ではなく既知の関係を見つけられるかを要求します。**インデックスはコミット単位です** — REAP 全体のインデックスが約 0.3 秒、HEAD が動けばクエリ自身が更新し、コミットされていない作業は意図的にインデックスしません。インデックスは `.reap/.index/` にあり gitignore されます。**`reap uninstall`** は npm が消せないものを消します — スラッシュコマンド、エージェント定義、`~/.reap/`、SessionStart フックの項目。npm 10 でも 12 でもアンインストールフックは実行されないため、`npm uninstall -g` はこれらを残します。 **配布される genome が、コードを変更する前に `environment/source-map.md` を読むようエージェントに指示するようになりました。** あわせて greenfield の `reap init` がそのファイルを作成します — これまでは作成しておらず、規則だけを移していれば新規プロジェクトのエージェントは毎回存在しないファイルを探すことになっていました。`summary.md` は毎セッション読み込まれますが source-map は読み込まれないため、構造ドキュメントが開かれないまま、それが説明するコードが編集され得ました。既存プロジェクトは自分の `genome/evolution.md` を所有しているので、規則は v0.17.6 の migration note を通じて届き、ファイル自体が無い場合の対応も併せて示します。 同じ note がそのファイルのもう半分も修正します — reflect が構造の記述を `summary.md` に保つよう指示していたため、そのままでは新しい規則が**誰も書くよう指示されていないファイル**を指すことになっていました。 **プロジェクトへ REAP を入れても、グローバルのインストールが変わることはなくなりました。** auto-update は「今インストールされているバージョン」を `reap --version` で尋ねていました — それは **PATH で最初に見つかるバイナリ**であって、そのコードが属するパッケージではありません — そのうえで無条件にグローバルのインストールを更新していました。古い REAP がグローバルにある状態でプロジェクトに入れると、postinstall が**ユーザーが一言も触れていないグローバル側**を更新していたのです。REAP は自分の `package.json` を読むようになり、自分自身を更新するのは**グローバルインストールのときだけ**です。プロジェクトローカル・`npx` 実行・ソースチェックアウトはそのまま残されます。 **そして `autoUpdate: false` が実際に自動更新を止めるようになりました。** このスイッチは v0.16 から `.reap/config.yml` にあり、`reap init` が作成し、`reap update` が保持し、`reap config` が表示し、`/reap.config` skill が \"auto-update enabled\" と記載し、`reap fix` が型まで検査していました — それでいて**どこからも読まれていませんでした**。`false` にすると `false` と表示されたまま、グローバルインストールが置き換えられていたのです。今は**インストールだけ**を止めます。ハードフロア警告 — あなたの版が古すぎて REAP が自動では直せないという通知 — はそのまま届きます。自動修正を断った人こそ、それを知る必要があるからです。`reap update` と、ご自身で打った `npm install -g` は影響を受けません。このフラグが止めるのは **REAP が自ら行うインストール一つだけ**です。config をまったく読めない場合、自動更新は有効のままにします。npm は postinstall をパッケージディレクトリで実行し、そこにプロジェクトの config は存在しないため、「読めない」を「オフ」と解釈すると、頼んだ人ではなく**全員**に対して無効になっていたはずです。 適用範囲を明記しておきます。このフラグが支配するのは**繰り返される経路 — 毎回のセッション開始**であって、ご自身で打った `npm install` の postinstall ではありません。そちらはプロジェクトの config が存在しない場所で走るからです。",
      },
      {
        version: "0.17.5",
        notes: "**`reap run push` が git の実際のエラーを報告**し、**`reap help` に `/reap.run` と `/reap.report`** が載りました。 **REAP が初回実行時に自身の統合をインストールします** — npm 12 がグローバルインストールの install script を遮断するため、スラッシュコマンド・エージェント定義・セッションフックが配置されず、エラーも出ませんでした。",
      },
      {
        version: "0.17.4",
      ...
      {
        version: "0.17.1",
        notes: "**Migration instruction layer** — `reap update`がプロジェクトの`lastMigratedVersion`とインストール済みREAPバージョンの差を検出し、該当するバージョン別のmigration指示をSessionStart contextに注入します。エージェントはアップグレード後に既存artifactを再編成するための具体的な指示を受け取り、適用後に`reap update --mark-migrated`を実行します。**Memory構造の改善** — vision memoryをcontent-typeベースの階層に移行し、reflectフェーズに必須のpruningポリシーを追加。",
      },
      {
        version: "0.17.0",
        notes: "**Evaluator Agent** — fitnessフェーズ統合完了: fitnessフェーズでevaluator実行、prior concernsをプロンプトに表示、high-severity懸念時にクルーズモードを自動停止。",
      },
      {
        version: "0.16.6",
      ...
      {
        version: "0.16.5",
        notes: "**OpenCodeアダプター** — `agentClient: opencode` でマルチクライアントを正式サポート。`opencode.json` の instructions、`.opencode/plugins/reap-plugin.ts`、AGENTS.md、`~/.config/opencode/commands/` のスラッシュコマンドを自動管理します。**Early-close ライフサイクルパス** — implementation/validation ステージから使える軽量な終了方法（`reap run early-close`、`/reap.early-close`）。部分的な成果を保存し、簡略化した reflect を実行して、未完了のタスクを次の generation 用 backlog へ自動的に引き継ぎます。ユーザーが中断やスコープ縮小の意図を示した場合、エージェントは abort / early-close / continue の3つの選択肢を明示的に提示します。",
      },
      {
        version: "0.16.4",
