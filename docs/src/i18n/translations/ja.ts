...
commit:  1a2b3c4`,
  ...
  releaseNotes: {
    ...
    versions: [
      {
        version: "0.17.8",
        notes: "**v0.18 が npm の `next` タグで公開されました** — 自動更新では届きません。`reap update` を実行すると、v0.18 のインストールとプロジェクト移行を案内するアップグレードエージェントがインストールされます。**v0.18 は既定で英語です** — `.reap/config.yml` に `language: ko` を設定すると韓国語になります。バージョン確認は1日1回に減り、セッション開始が速くなります。",
      },
      {
        version: "0.17.7",
        notes: "**CLI に変更はありません。** 配布される `genome/evolution.md` にチェックの書き方の規則が二つ追加されました。既存のプロジェクトには `reap update` が移行ノートで案内します。",
      },
      {
        version: "0.17.6",
