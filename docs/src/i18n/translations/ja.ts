...
commit:  1a2b3c4`,
  ...
  releaseNotes: {
    ...
    versions: [
      {
        version: "0.17.8",
        notes: "**v0.18 が npm の `next` タグで公開されました** — 自動更新では届きません。`reap update` を実行すると、v0.18 のインストールとプロジェクト移行を案内するアップグレードエージェントがインストールされます。**v0.18 は韓国語専用**です — 他の言語で作業している場合は 0.17 系にとどまってください。バージョン確認は1日1回に減り、セッション開始が速くなります。",
      },
      {
        version: "0.17.7",
