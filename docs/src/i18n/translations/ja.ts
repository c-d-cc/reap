...
export const ja: Translations = {
  ...
  concepts: {
    ...
    stages: [
      ...
      ["Implementation", "AI + 人間の協力で実装", "03-implementation.md"],
      ["Validation", "テスト実行、完了基準の検証", "04-validation.md"],
      ["Completion", "レトロスペクティブ + Genome更新 + 圧縮", "05-completion.md"],
    ],
...
# npm run lint`,
  ...
  environmentPage: { title: "Environment", breadcrumb: "ガイド", intro: "Environmentはプロジェクトに影響する外部コンテキストを格納します — API、インフラ、組織ルール、参考資料。", structureTitle: "3-Layer構造", structure: `.reap/environment/\n├── summary.md      # セッションコンテキスト (~100行、自動ロード)\n├── docs/           # 主要参考ドキュメント\n└── resources/      # 原本資料 (ユーザー管理)`, layersTitle: "レイヤー", layerHeaders: ["レイヤー", "管理主体", "内容", "制限"], layerItems: [["summary.md", "AI (自動生成)", "docs/全体の概要。毎セッションにロード。", "~100行"], ["docs/", "AI + ユーザー", "環境トピック別ファイル。", "ファイルあたり~100行"], ["resources/", "ユーザー", "原本ドキュメント、PDF、外部リンク。", "制限なし"]], flowTitle: "参照フロー", flowDesc: "summary.md (常にロード) → docs/ (詳細が必要な場合) → resources/ (原本が必要な場合)", syncTitle: "同期", syncDesc: "/reap.sync.environmentでソースから外部依存関係を発見し、ユーザーインタビューで文書化します。" },
  lifecyclePage: { title: "ライフサイクル", breadcrumb: "ガイド", intro: "ライフサイクルはREAPの心臓部です — 各Generationは5段階（Objective → Planning → Implementation → Validation → Completion）を経て、各ステップで成果物を生成します。AIエージェントがライフサイクル全体を案内します。" },
