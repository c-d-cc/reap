...
export const ja: Translations = {
  ...
  hero: {
    ...
    installStep3: "3. Genome同期 & 最初のGenerationを実行（Claude Codeで）",
    installNote: [
  ...
  cli: {
    ...
    runNote: "31個のスラッシュコマンドすべてがこのアーキテクチャを使用します：.mdファイルがreap run <cmd>を呼び出す1行wrapperで、TSスクリプトがAIエージェントにstructured JSON指示を返します。",
    ...
    fixTitle: "reap fix",
    fixDesc: ".reap/ディレクトリ構造を診断し修復します。--checkで読み取り専用モードが使用可能。",
    fixNote: "不足ディレクトリの確認、config.ymlの存在検証、current.ymlステージの検証、不足構造の再作成を行います。--check使用時は変更なしで構造的整合性のみチェックします。",
    cleanTitle: "reap clean",
    cleanDesc: "対話式オプションでREAPプロジェクトをリセットします。",
    cleanNote: "REAPプロジェクトの特定部分（例：life、lineage、genome）を選択的にリセットする対話式プロンプトを提供します。",
    destroyTitle: "reap destroy",
    destroyDesc: "プロジェクトからすべてのREAPファイルを削除します。",
    destroyNote: "プロジェクトから.reap/ディレクトリとすべてのREAP関連ファイルを完全に削除します。\"yes destroy\"の入力で確認が必要です。",
    helpTitle: "reap help",
  ...
  commands: {
    ...
    normalCommands: [
      ...
      ["/reap.evolve.recovery", "失敗または中断したGenerationから復旧。問題が発生した箇所から引き継ぐrecovery Generationを作成。"],
    ...
    generalTitle: "一般",
    generalCommands: [
      ["/reap.abort", "現在のGenerationを中断。コード変更に対してrollback/stash/holdを選択し、abort メタデータ付きでbacklogに保存可能。"],
      ["/reap.status", "現在のGeneration状態、ステージ進捗、backlog概要、タイムライン、Genome状態を表示。"],
      ["/reap.sync", "GenomeとEnvironmentを同時に同期。sync.genome + sync.environmentを順次実行。"],
      ["/reap.sync.genome", "ソースコードを分析しGenomeを同期。アクティブGenerationがなければ直接更新；あればbacklogに記録。"],
      ["/reap.sync.environment", "外部依存関係（API、インフラ、サービス）を探索し、3層環境構造（summary.md + docs/ + resources/）に文書化。"],
      ["/reap.config", "現在のプロジェクト設定（.reap/config.yml）を表示。"],
