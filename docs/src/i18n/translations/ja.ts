...
# reap update`,
    ...
    sessionStart: "セッションコンテキストの読み込み",
    sessionStartDesc1: "REAPはCLAUDE.mdにセクションを追加し、毎セッション開始時にAIエージェントにgenome、environment、REAPガイドを読み込むよう指示します。これにより、エージェントは常にプロジェクトのフルコンテキストを持ちます。",
    sessionStartDesc2: "REAPガイド（~/.reap/reap-guide.md）はグローバルにインストールされ、パッケージのアップグレード時に自動更新されます。CLAUDE.mdは'reap init'中に作成または更新されます。",
    sessionStartNote: "",
