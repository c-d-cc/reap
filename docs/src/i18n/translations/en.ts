...
commit:  1a2b3c4`,
  ...
  releaseNotes: {
    ...
    versions: [
      ...
      {
        version: "0.17.6",
        notes: "**`reap index` — a code index is built in.** Nothing to install, no port, no background process: `reap index status | search | impact | callers | callees`. Fifteen languages, no native build. The index is keyed by commit, lives in `.reap/.index/` and is gitignored, so uncommitted work is deliberately not in it.\n\n**`reap uninstall`** removes what npm cannot — slash commands, agent definitions, `~/.reap/` and the SessionStart hook entries, which `npm uninstall -g` leaves behind. Already removed the package? `npx @c-d-cc/reap uninstall --confirm`.\n\n**Installing REAP into a project no longer changes your global install**, and **`autoUpdate: false` now actually turns auto-update off** — the setting had been in `.reap/config.yml` since v0.16 and nothing read it.\n\n**The daemon is retired.** `reap update` removes `daemon` / `daemonBin` from your config and deletes `~/.reap/daemon/`. Remove the global package with `npm uninstall -g @c-d-cc/reap-daemon`.",
      },
      {
        version: "0.17.5",
        notes: "**REAP installs its own integration on first run.** npm 12 blocks install scripts for global installs, which left slash commands, agent definitions and the session hook unplaced — with no error. The binary worked and the integration was simply absent. **`reap run push` now reports git's actual error** instead of a guess, and **`reap help` lists `/reap.run` and `/reap.report`**.",
      },
      {
        version: "0.17.4",
