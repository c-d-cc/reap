export const en = {
  ...
  cli: {
    ...
    uninstallDesc: "Remove REAP from this machine — user-level files and the npm packages.",
    uninstallNote: "npm removes the package and nothing else: REAP's slash commands, agent definitions, SessionStart hook and ~/.reap/ were written by REAP's own code, and npm runs no code at uninstall time. This does it in the right order — clears both clients' user-level files, takes only REAP's entries out of settings.json, removes what REAP put in ~/.reap/, then hands its npm packages to npm. Your own files survive, including reapdev.* and anything else in ~/.reap/. Already removed the package? npx @c-d-cc/reap uninstall --confirm. Not to be confused with reap destroy, which removes REAP from one project.",
...
Description of the task.`,
  ...
  codeIntelligencePage: {
    ...
    commandsDesc: "Every subcommand emits JSON on stdout like the rest of REAP, so an agent can parse it and a human can read the message field.",
    commandsCode: `reap index                     # update — the default
...
reap index callees <symbolId>  # what this calls`,
    commandsNote: "A symbolId is <file>::<name> — for example src/core/lifecycle.ts::nextStage. reap index search prints them.",
    commitTitle: "The unit of change is a commit",
    commitDesc: "The index records the SHA it describes. Deciding what to re-parse is one git diff, and deciding whether to bother is one string comparison. Queries refresh the index themselves when HEAD has moved — after a commit, a branch switch, a rebase or a pull — so REAP refreshes eagerly only where it has just committed: at the end of completion, and in early-close.",
...
reap index update    # Index is current at 1a2b3c4 (1530 symbols, 3688 edges)`,
    commitNote: "The trade-off: uncommitted work is not in the index. A symbol you wrote a minute ago and have not committed will not be found by search, and a new file will not appear in impact — use Grep for those. A git repository is required, since without one there is no commit to key the index to.",
    statusTitle: "Read status before trusting impact",
    statusDesc: "Everything impact knows comes from resolved import edges. A low imports rate means the graph is incomplete, and an empty blast radius then means unknown rather than none. status also warns when a grammar fails to load, which is the other way a language goes silently unindexed.",
...
commit:  1a2b3c4`,
    ...
    whenDesc: "The two are complementary — the index answers with file:line, which you then read.",
    whenHeaders: ["Reach for", "When the question is"],
    whenItems: [
      ["reap index", "Where is this defined, who calls it, what depends on this file, what is the blast radius of this change"],
      ["Grep / Glob", "Literal strings, comments, config files, languages with no grammar, and anything not committed yet"],
    ],
  },
  ...
  selfEvolvingPage: {
    ...
    breadcrumb: "Guide",
    intro: "REAP is not a static framework. The AI automatically selects goals from vision and memory, the human judges fitness through natural language feedback, and the pipeline adjusts its communication style based on context clarity. These features work together to create a development pipeline that genuinely evolves alongside your project.",
    gapDrivenTitle: "Gap-Driven Goal Selection",
    gapDrivenDesc: "At the end of each generation (adapt phase), the AI analyzes the gap between the project's vision and its current state to propose the next goal. This is the core mechanism that makes REAP self-evolving.",
    gapDrivenSteps: [
    ...
    gapDrivenNote: "In cruise mode, goal selection happens automatically between generations without human intervention.",
    fitnessTitle: "Human Judges Fitness",
    fitnessDesc: "There are no quantitative metrics. The human's natural language feedback during the fitness phase is the only fitness signal. The AI is explicitly prohibited from scoring its own success — only self-assessment (metacognition) is allowed.",
    fitnessNote: "This ensures the project evolves toward what the human values, not toward what the AI can optimize for.",
    clarityTitle: "Clarity-Driven Interaction",
    clarityDesc: "The AI dynamically assesses how well-defined the current context is and adjusts its communication depth accordingly. Clarity is evaluated at the start of each generation during the learning stage.",
    clarityHeaders: ["Clarity", "Signals", "AI Behavior"],
    clarityRows: [
    ...
    cruiseTitle: "Cruise Mode",
    cruiseDesc: "Pre-approve N generations for autonomous execution. The AI selects goals from vision gaps and backlog tasks, runs the full lifecycle, and self-assesses (not self-scores) each generation.",
    cruiseActivation: "Activation:",
    cruiseActivationDesc: "Sets cruiseCount: 1/5 in config.yml. The counter increments after each generation completes.",
    cruiseGoalSelection: "Goal selection:",
    cruiseGoalSelectionDesc: "During adapt, the AI analyzes gaps between vision/goals.md and the current state, then picks the highest-value next goal.",
    cruiseFitness: "Fitness:",
    cruiseFitnessDesc: "In cruise mode, the fitness phase uses self-assessment (metacognition) instead of waiting for human feedback. The AI is explicitly prohibited from scoring its own success.",
    cruisePause: "Pause conditions:",
    ...
    visionEvolutionTitle: "Vision & Gap-Driven Evolution",
    visionEvolutionDesc: "Vision is the primary driver of each generation. The file .reap/vision/goals.md defines north-star objectives for the project. During the adapt phase, the AI performs gap analysis: comparing vision goals against the current state of the codebase.",
    visionSteps: [
      { title: "1. Goals Definition", desc: "The human writes high-level objectives in goals.md. Each goal is a markdown checkbox item. Goals can be nested for sub-objectives." },
      { title: "2. Gap Analysis (adapt phase)", desc: "The AI compares unchecked goals against the current codebase, environment, and generation history. It identifies the highest-value gap and suggests it as the next generation's goal." },
      { title: "3. Auto Check-Marking", desc: "When a generation achieves a vision goal, the AI marks the corresponding checkbox in goals.md during the adapt phase. This provides a persistent view of project progress." },
      { title: "4. Continuous Cycle", desc: "Vision evolves too. The human updates goals as the project's direction changes. The AI references both completed and remaining goals to maintain trajectory." },
    ],
  },
  ...
  visionPage: {
    title: "Vision",
    breadcrumb: "Guide",
    intro: "Vision is the layer that drives direction. The human defines where the project is going — setting goals, milestones, and priorities. The AI references vision during the adapt phase to analyze gaps and propose the next goal, but the human owns the vision.",
    structureTitle: "Structure",
    goalsTitle: "Goals",
    goalsDesc: "goals.md contains the project's long-term objectives as a checklist. During the adapt phase, the AI analyzes gaps between vision and current state to suggest the next generation's goal. Completed goals are automatically checked off.",
    memoryTitle: "Memory",
    memoryDesc: "Memory is a free-form recording system where AI persists context across sessions and generations. Unlike genome (which has modification constraints) or lineage (which gets compressed), memory is always accessible and freely writable.",
    memoryHeaders: ["Tier", "Role", "Decision rule"],
    memoryRows: [
    ...
    whenToUpdateTitle: "When to Update",
    whenToUpdateItems: [
      { label: "Reflect phase", desc: "natural moment to update memory (shortterm is mandatory every generation)" },
      { label: "Any time", desc: "memory can be updated during any stage if useful context arises" },
      { label: "Shortterm cleanup", desc: "clear items that have been acted on" },
    ],
    whatNotTitle: "What NOT to Write in Memory",
    whatNotItems: [
    ...
    gapDrivenTitle: "Gap-driven Evolution",
    gapDrivenDesc: "During the adapt phase of completion, the AI:",
    gapDrivenSteps: [
  ...
  migrationGuidePage: {
    title: "Migration Guide (v0.15 → v0.16)",
    breadcrumb: "Other",
    intro: "REAP v0.16 is a complete rewrite built on the Self-Evolving Pipeline architecture. The lifecycle, genome structure, configuration, and vision system have all been redesigned. A guided migration tool handles the transition automatically.",
    whyMigrateTitle: "Why Migrate?",
    whyMigrateItems: [
    ...
    stepsTitle: "Migration Steps",
    step1: "Install v0.16 globally. This automatically deploys v0.16 skills and removes legacy v0.15 project-level skills.",
    step2: "Open Claude Code in your project and run the migration command.",
    step3: "Follow the 5-phase guided migration:",
    step3Headers: ["Phase", "What Happens", "Your Role"],
    step3Rows: [
    ...
    step4: "Verify the migration succeeded.",
    whatChangesTitle: "What Changes",
    whatChangesHeaders: ["Area", "v0.15", "v0.16"],
    whatChangesRows: [
    ...
    interruptedTitle: "Interrupted Migration",
    interruptedDesc: "If the migration is interrupted (API error, session disconnect, etc.), progress is saved in .reap/migration-state.yml. This file tracks which phases have completed.",
    interruptedResume: "To resume:",
    interruptedResumeDesc: "Run /reap.update again. It skips completed phases and continues from where it left off.",
    interruptedRestart: "To restart:",
    interruptedRestartDesc: "Delete .reap/migration-state.yml and run /reap.update again.",
    backupTitle: "Backup",
    backupDesc: "All v0.15 files are preserved at .reap/v15/ during the confirm phase. This is a complete snapshot of your pre-migration state.",
    backupItems: [
    ...
    deprecatedTitle: "Deprecated Commands",
    deprecatedDesc: "Several v0.15 slash commands have been renamed or consolidated in v0.16.",
    deprecatedHeaders: ["v0.15 Command", "v0.16 Replacement", "Notes"],
    deprecatedRows: [
    ...
    deprecatedNote: "Legacy v0.15 project-level skill files are automatically removed during npm install. If any remain in .claude/commands/, they can be safely deleted.",
  },
  ...
  releaseNotes: {
    ...
    versions: [
      {
        version: "0.17.7",
        notes: "**The documentation site was serving one empty shell for every page.** `reap.cc/docs/*` returned HTTP 404 for every address and had since the site went up — a single-page app on GitHub Pages, so every URL got the same 939-byte shell, the shell booted, the router drew the right page, and a human saw a perfect site. Search engines saw a 404 and indexed nothing but the root. **Every page is now rendered at build time**: 23 routes across five languages, 115 files, each with its own content, `<title>`, `<meta description>`, canonical URL and `hreflang` alternates, plus a `sitemap.xml` and a `robots.txt` that did not exist. The smallest page went from 939 bytes to 15 kB. **Languages have their own addresses.** English keeps the ones it had — `reap.cc/docs/quick-start` — and the other four are prefixed: `/ko/docs/quick-start`, `/ja/...`, `/de/...`, `/zh-CN/...`. Before this the five shared one URL and your browser chose, so a crawler could index only one of them and a link you shared showed a different language to whoever opened it. Landing on `/` still sends you to your own language once per tab; every other address is left exactly as you asked for it. **The retired daemon is gone from the documentation, not just from the code** — including a migration note that was injecting `npm i -g @c-d-cc/reap-daemon`, a deprecated package, into every session's context in projects that had never enabled it. **`/docs/daemon` is now `/docs/code-intelligence`; a bookmark to the old address will stop working.** Nothing about the CLI changed.",
      },
      {
        version: "0.17.6",
        notes: "**REAP ships a code index.** Nothing to install, no port, no background process — `reap index status`, `reap index impact <file>`, `reap index search <query>`, `reap index callers/callees <symbolId>`. Fifteen languages, no native build. **Blast radius used to return zero for every standard TypeScript project**, and had for five months: the resolver never mapped a `./x.js` specifier to the `x.ts` that produces it. 130 tests passed and CI stayed green throughout, because every check asked whether indexing had run and none asked whether the answer meant anything. **So `reap index status` now reports the import resolution rate**, and the release gate requires a known relationship to be found rather than a nonzero symbol count. **Indexing is keyed by commit** — a full index of REAP takes ~0.3s, queries refresh themselves when HEAD moves, and uncommitted work is deliberately not indexed. The index lives in `.reap/.index/` and is gitignored. **`reap uninstall`** removes what npm cannot — slash commands, agent definitions, `~/.reap/` and the SessionStart hook entries, which `npm uninstall -g` leaves behind because uninstall hooks do not fire on npm 10 or 12. **The shipped genome now tells agents to read `environment/source-map.md` before changing code**, and `reap init` creates that file for greenfield projects — it never did, so propagating the rule alone would have pointed every new project's agent at a file that does not exist. `summary.md` is loaded every session and source-map is not, which is how a structure document sits unopened while the code it describes is edited. Existing projects own their `genome/evolution.md`, so the rule reaches them through the v0.17.6 migration note, which also says what to do when the file itself is missing. The same note also fixes the other half of that file: it told the reflect phase to keep the structure description in `summary.md`, which would have left the new rule pointing at a file nothing is told to write. **Installing REAP into a project no longer changes your global install.** The auto-update asked `reap --version` for the currently installed version — that is whatever binary is first on PATH, not the package the code belongs to — and then upgraded the global installation unconditionally. Install REAP into a project while an older one sits globally and the postinstall upgraded the global one, which you never mentioned. REAP now reads its own `package.json`, and only a global installation upgrades itself: a project-local copy, an `npx` run and a source checkout are left alone. **And `autoUpdate: false` now turns it off.** The switch has been in `.reap/config.yml` since v0.16 — created by `reap init`, preserved by `reap update`, printed by `reap config`, documented in the `/reap.config` skill as \"auto-update enabled\", type-checked by `reap fix` — and read by nothing. Setting it `false` showed you `false` and replaced your global install anyway. It now stops the install, and only the install: the hard-floor warning that says your copy is too old for REAP to fix automatically still reaches you, because the person who declined automatic fixing is the one who most needs to hear it. `reap update` and an `npm install -g` you typed yourself are unaffected — the flag governs the single install REAP performs on its own. When the config cannot be read at all, auto-update stays on; npm runs postinstall in the package directory, where there is no project config, so reading \"cannot tell\" as \"off\" would have disabled it for everyone rather than for the people who asked. Say plainly what that means for reach: the flag governs the recurring path — every session start — and not the postinstall of an `npm install` you just typed, which runs where your project's config is not.",
      },
      {
        version: "0.17.5",
        notes: "**`reap run push` reports git's actual error**, and **`reap help` lists `/reap.run` and `/reap.report`**. **REAP installs its own integration on first run** — npm 12 blocks install scripts for global installs, which left slash commands, agent definitions and the session hook unplaced with no error at all.",
      },
      {
        version: "0.17.4",
      ...
      {
        version: "0.17.1",
        notes: "**Migration instruction layer** — `reap update` detects the gap between a project's `lastMigratedVersion` and the installed REAP version, then injects the matching per-version migration notes into the agent's SessionStart context. Agents get actionable instructions for reorganizing existing artifacts after an upgrade; run `reap update --mark-migrated` once applied. **Memory structure** — vision memory moved to content-type-based tiers with a mandatory pruning policy in the reflect phase.",
      },
      {
        version: "0.17.0",
        notes: "**Evaluator Agent** — fitness phase integration complete: evaluator runs during fitness, prior concerns surface in prompt, high-severity concerns automatically abort cruise mode.",
      },
      {
        version: "0.16.6",
