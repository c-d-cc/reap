...
commit:  1a2b3c4`,
  ...
  releaseNotes: {
    ...
    versions: [
      {
        version: "0.17.8",
        notes: "**v0.18 is out on the npm `next` tag** — it will not arrive via auto-update. `reap update` installs a guided upgrade agent that walks you through installing v0.18 and migrating your project. **v0.18 speaks English by default**; set `language: ko` in `.reap/config.yml` for Korean. Version checks now run at most once a day, so sessions start faster.",
      },
      {
        version: "0.17.7",
      ...
      {
        version: "0.17.6",
        notes: "**A code index is built in.** `reap index`, and nothing to install, no port, no background process: `reap index status | search | impact <file> | callers <symbolId> | callees <symbolId>`. Fifteen languages, no native build. The index is keyed by commit, lives in `.reap/.index/` and is gitignored, so uncommitted work is deliberately not in it.\n\n**`reap uninstall` removes what npm cannot.** Slash commands, agent definitions, `~/.reap/` and the SessionStart hook entries survive `npm uninstall -g`, and the leftover hook kept calling a command that no longer existed. Already removed the package? `npx @c-d-cc/reap uninstall --confirm`.\n\n**Installing REAP into a project no longer changes your global install.** A project-local copy, an `npx` run and a source checkout now leave your machine alone.\n\n**`autoUpdate: false` now actually turns auto-update off.** The setting has been in `.reap/config.yml` since v0.16 and nothing read it. It stops the install and only the install — the warning that your copy is too old to fix automatically still reaches you.",
      },
      {
        version: "0.17.5",
        notes: "**REAP installs its own integration on first run.** npm 12 blocks install scripts for global installs, which left slash commands, agent definitions and the session hook unplaced with no error at all — the binary worked and the integration was simply absent.\n\n**`reap run push` reports git's actual error** instead of a guess.\n\n**`reap help` lists `/reap.run` and `/reap.report`.**",
      },
      {
        version: "0.17.4",
