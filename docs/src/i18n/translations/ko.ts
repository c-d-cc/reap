...
strict:
  ...
  hooks: {
    ...
    eventItems: [
      ...
      ["onGenomeResolved", "병합 Generation에서 Genome 충돌이 해결된 후"],
...
# npm run lint`,
  ...
  mergeCommands: {
    ...
    mergeHookItems: [
      ["onMergeStart", "/reap.merge.start가 병합 Generation을 생성한 후"],
      ["onGenomeResolved", "Genome 충돌이 해결된 후 (mate → merge 전환)"],
      ["onMergeComplete", "병합 Generation이 보관된 후"],
    ],
  ...
  environmentPage: { title: "Environment", breadcrumb: "가이드", intro: "Environment는 프로젝트에 영향을 미치는 외부 컨텍스트를 저장합니다 — API, 인프라, 조직 규칙, 참고 자료.", structureTitle: "3-Layer 구조", structure: `.reap/environment/\n├── summary.md      # 세션 컨텍스트 (~100줄, 자동 로딩)\n├── docs/           # 주요 참고 문서\n└── resources/      # 원본 자료 (유저 관리)`, layersTitle: "레이어", layerHeaders: ["레이어", "관리 주체", "내용", "제한"], layerItems: [["summary.md", "AI (자동 생성)", "docs/ 전체 개요. 매 세션에 로딩.", "~100줄"], ["docs/", "AI + 유저", "환경 주제별 파일.", "파일당 ~100줄"], ["resources/", "유저", "원본 문서, PDF, 외부 링크.", "제한 없음"]], flowTitle: "탐색 흐름", flowDesc: "summary.md (항상 로딩) → docs/ (상세 필요 시) → resources/ (원본 필요 시)", syncTitle: "동기화", syncDesc: "/reap.sync.environment로 소스에서 외부 의존성을 탐색하고 유저 인터뷰를 통해 문서화합니다." },
  lifecyclePage: { title: "라이프사이클", breadcrumb: "가이드", intro: "라이프사이클은 REAP의 심장입니다 — 각 Generation은 5단계(Objective → Planning → Implementation → Validation → Completion)를 거치며, 매 단계에서 산출물을 생성합니다. AI 에이전트가 전체 라이프사이클을 안내합니다." },
