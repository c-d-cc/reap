...
commit:  1a2b3c4`,
  ...
  releaseNotes: {
    ...
    versions: [
      {
        version: "0.17.7",
        notes: "**문서 사이트가 모든 페이지에 빈 셸 하나를 내주고 있었습니다.** `reap.cc/docs/*` 가 모든 주소에서 HTTP 404 를 반환했고 사이트가 올라간 이래 계속 그랬습니다 — GitHub Pages 위의 SPA 라 모든 URL 이 같은 939바이트 셸을 받고, 셸이 부팅해 라우터가 올바른 페이지를 그리므로 **사람 눈에는 완전한 사이트**였습니다. 검색엔진은 404 를 보고 루트 외에 아무것도 색인하지 않았습니다. **이제 모든 페이지가 빌드 시점에 렌더됩니다**: 5개 언어 × 23 라우트 = **115개 파일**, 각각 자기 본문·`<title>`·`<meta description>`·canonical·`hreflang` 을 갖고, 존재하지 않던 `sitemap.xml` 과 `robots.txt` 도 생겼습니다. 가장 작은 페이지가 939바이트에서 15 kB 가 됐습니다. **언어마다 주소가 생겼습니다.** 영어는 쓰던 주소를 그대로 씁니다 — `reap.cc/docs/quick-start` — 나머지 넷은 접두사를 갖습니다: `/ko/docs/quick-start`, `/ja/...`, `/de/...`, `/zh-CN/...`. 이전에는 다섯 언어가 한 URL 을 공유하고 브라우저가 골랐기 때문에, 크롤러는 그중 하나만 색인할 수 있었고 공유한 링크는 여는 사람마다 다른 언어를 보여줬습니다. `/` 에 처음 들어오면 여전히 탭당 한 번 자기 언어로 이동하고, 그 외 주소는 요청하신 그대로 둡니다. **폐기된 daemon 이 코드뿐 아니라 문서에서도 사라졌습니다** — 그중에는 `npm i -g @c-d-cc/reap-daemon`(deprecated 패키지) 설치를 지시하며 daemon 을 켠 적 없는 프로젝트의 매 세션 컨텍스트에 주입되던 migration note 도 있었습니다. **`/docs/daemon` 은 `/docs/code-intelligence` 가 됐습니다. 옛 주소를 북마크해 두셨다면 더 이상 열리지 않습니다.** CLI 는 달라진 것이 없습니다.",
      },
      {
        version: "0.17.6",
