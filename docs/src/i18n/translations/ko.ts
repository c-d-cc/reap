...
commit:  1a2b3c4`,
  ...
  releaseNotes: {
    ...
    versions: [
      {
        version: "0.17.8",
        notes: "**v0.18 이 npm `next` 태그로 나왔습니다** — 자동 업데이트로는 오지 않습니다. `reap update` 를 실행하면 v0.18 설치와 프로젝트 이주를 안내하는 upgrade agent 가 설치됩니다. **v0.18 은 기본이 영어**이며, `.reap/config.yml` 에 `language: ko` 를 두면 한국어로 씁니다. 버전 확인이 하루 한 번으로 줄어 세션 시작이 빨라집니다.",
      },
      {
        version: "0.17.7",
