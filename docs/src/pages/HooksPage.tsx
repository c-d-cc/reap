  ...
  return (
    <DocLayout>
      <DocPage title={t.hooks.title} breadcrumb={t.hooks.breadcrumb}>
        ...
        <h2 className="text-base font-semibold text-foreground mb-3 mt-6">{t.hooks.normalEventsTitle}</h2>
        <div className="border border-border rounded-md overflow-hidden text-sm mb-6">
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
            ...
            <tbody className="divide-y divide-border">
              {t.hooks.eventItems.slice(0, 7).map(([event, when]) => (
                <tr key={event}>
        ...
        <h2 className="text-base font-semibold text-foreground mb-3 mt-6">{t.hooks.mergeEventsTitle}</h2>
        <div className="border border-border rounded-md overflow-hidden text-sm mb-6">
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
            ...
            <tbody className="divide-y divide-border">
              {t.hooks.eventItems.slice(7).map(([event, when]) => (
                <tr key={event}>
                  <td className="px-4 py-2 font-mono text-xs text-primary">{event}</td>
                  <td className="px-4 py-2 text-xs text-muted-foreground">{when}</td>
                </tr>
