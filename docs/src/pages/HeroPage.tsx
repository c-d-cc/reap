      ...
      <main className="flex-1 pt-11 md:pt-14 overflow-hidden flex justify-center">
        ...
        <Section title={t.hero.lifecycle}>
          ...
          <div className="flex items-center gap-1.5 flex-wrap mb-5">
            {["Learning", "→", "Planning", "→", "Implementation", "⟷", "Validation", "→", "Completion"].map((s, i) =>
              s === "→" || s === "⟷"
        ...
        <Section title={t.hero.installation}>
          <div className="space-y-3">
            <div>
              ...
              <CodeBlock language="bash">{`npm install -g @c-d-cc/reap   # Global installation required`}</CodeBlock>
            </div>
            <div>
...
> /reap.evolve        # Run a full generation lifecycle`}</CodeBlock>
            </div>
          </div>
          <p className="text-xs text-muted-foreground mt-2">
            <code className="bg-muted px-1 rounded">{t.hero.installNote[0].code}</code>{t.hero.installNote[0].after}
            <Link href="/docs/lifecycle" className="text-primary hover:underline">{t.hero.installNote[1].linkText}</Link>{t.hero.installNote[1].after}
          </p>
        </Section>
