...
쓰는 순간은 **사람에게 돌려주는 때**다. REAP는 그 순간을 감지할 수 없다 — 세션 종료 이벤트가 없고, 있다 해도 agent가 멈출지는 판단이다. 그러므로 이것은 도구가 아니라 skill의 일이다. **절차는 `handoff` skill이 갖고 사람이 직접 부를 수 있다**; `complete`는 닫은 뒤 멈추기로 했을 때 그것을 부른다.

멈추기로 했으면 한 번 더 묻는다. **이번 세션이 한 일 중, 그것을 알아야 다음 세션이 이어서 일할 수 있는 것이 있는가.**

- 있다 → 그것을 내 절에 적는다(교체한다)
- 없다 → **쓰지 않는다.** 적을 것이 없으면 구태여 적지 않는다

**"막히는가"로 넓게 묻지 않는다.** 그렇게 물으면 사람의 답을 기다리는 것, 아직 안 정한 것, 남은 일이 전부 걸려든다. 가르는 축은 **누구의 무엇인가**다 — **이번 세션이 한 일**이고, **이어서 일하는 데 필요한 것**이라야 한다.

그러므로 이것들은 인계가 아니다.

| 인계가 아닌 것 | 어디가 제자리인가 |
|---|---|
| 열린 milestone, 사람 검수·fitness 대기 | milestone이 열려 있다는 사실이 말한다. 상태 줄이 알린다 |
| 열린 flux, 열린 backlog 항목 | 각자의 파일과 상태 줄 |
| 결론 안 난 물음 | `idea/research/` |
| 반복 확인된 교훈 | `vision/memory/lessons.md` |
| 이번 세션이 끝낸 일의 요약 | 세대 기록의 `Outcome`과 커밋 |

이것들을 인계에 옮겨 적으면 **읽는 쪽이 무엇이 진짜 이어받을 것인지 못 가린다.**
