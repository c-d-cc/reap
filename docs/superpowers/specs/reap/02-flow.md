...
**보고는 이미 파일에 있다.** 세대 기록의 `Outcome`과 이월된 backlog·idea까지 `complete`가 전부 쓴다. 산문으로 다시 말하는 것은 넷째 사본이고, 그 사본의 대가가 실행 종료다.
...
멈추는 것이 맞는 경우도 분명하다. 남은 항목이 없을 때, 사람이 건 멈춤 조건이 충족됐을 때, 그리고 **사람의 것**이 나왔을 때다 — 커밋 규칙이 어긋났거나, milestone fitness처럼 사람만 답할 수 있는 것이 앞에 놓였을 때. 무엇이 사람의 것인지는 프로젝트가 자기 훅에 적는다.

**인계를 쓰는 자리가 여기다** — 멈추기로 한 뒤, 다음 세션이 모르면 곤란한 것이 있을 때만([인계](03-storage.md#언제-쓰는가--세션이-끝날-때-필요할-때만)). 계속 갈 참이면 쓰지 않는다. **멈춤은 실패가 아니다** — 틀린 것은 남은 일이 있는데 멈추는 것뿐이다.
...
| 세대 닫기 | `complete` | `mark generation --closed` | 커밋, 마무리된 기록 |
| 사람에게 돌려주기 | `complete` | — | `life/handoff.md`의 내 절 — **이어받을 것이 있을 때만** |
