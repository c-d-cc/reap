...
## v0.17.7
### en
The documentation site at `reap.cc` was serving one 939-byte empty shell for every page, and returning HTTP 404 while doing it — a single-page app on GitHub Pages, so the shell booted and a human saw a perfect site while search engines indexed nothing but the root. Every page is now rendered at build time: 23 routes across five languages, 115 files, each with its own content, `<title>`, `<meta description>`, canonical URL and `hreflang` alternates, plus a `sitemap.xml` and `robots.txt` that did not exist. Languages have their own addresses — English keeps `reap.cc/docs/quick-start` and the others are prefixed (`/ko/…`, `/ja/…`, `/de/…`, `/zh-CN/…`) — where before the five shared one URL and your browser chose, so a shared link showed a different language to whoever opened it. Landing on `/` still sends you to your own language once per tab; every other address is left as you asked for it. The retired daemon is also gone from the documentation rather than merely from the code — including a migration note that was injecting `npm i -g @c-d-cc/reap-daemon`, a deprecated package, into every session's context in projects that had never enabled it. **`/docs/daemon` is now `/docs/code-intelligence`; a bookmark to the old address will stop working.** Nothing about the CLI changed.
### ko
`reap.cc` 문서 사이트가 모든 페이지에 **939바이트짜리 빈 셸 하나**를 내주면서 HTTP 404 를 반환하고 있었습니다 — GitHub Pages 위의 SPA 라 셸이 부팅해 사람 눈에는 완전한 사이트였지만, 검색엔진은 루트 외에 아무것도 색인하지 못했습니다. 이제 모든 페이지가 빌드 시점에 렌더됩니다: 5개 언어 × 23 라우트 = **115개 파일**, 각각 자기 본문·`<title>`·`<meta description>`·canonical·`hreflang` 을 갖고, 존재하지 않던 `sitemap.xml` 과 `robots.txt` 도 생겼습니다. 언어마다 주소가 생겼습니다 — 영어는 `reap.cc/docs/quick-start` 를 그대로 쓰고 나머지는 접두사(`/ko/…`, `/ja/…`, `/de/…`, `/zh-CN/…`)를 갖습니다. 이전에는 다섯 언어가 한 URL 을 공유하고 브라우저가 언어를 정했기 때문에, 공유한 링크가 여는 사람마다 다른 언어를 보여줬습니다. `/` 에 처음 들어오면 여전히 탭당 한 번 자기 언어로 이동하며, 그 외 주소는 요청한 그대로 둡니다. 폐기된 daemon 이 코드뿐 아니라 **문서에서도** 사라졌습니다 — 그중에는 `npm i -g @c-d-cc/reap-daemon`(deprecated 패키지) 설치를 지시하며 daemon 을 켠 적 없는 프로젝트의 매 세션 컨텍스트에 주입되던 migration note 도 있었습니다. **`/docs/daemon` 은 `/docs/code-intelligence` 가 됐습니다. 옛 주소를 북마크해 두셨다면 더 이상 열리지 않습니다.** CLI 는 달라진 것이 없습니다.
