...
## v0.17.5
### en
If you use `daemon: true`, install the daemon once: `npm i -g @c-d-cc/reap-daemon`. It was documented since v0.16 but never published — npm installs got a dangling symlink and every daemon call failed silently while the lifecycle carried on. Three defects sat behind that: the bundle inlined its native bindings so it only started under bun, it looked for its Tree-sitter queries one directory too far up so indexing extracted zero symbols, and `__dirname` was baked in at build time — the published v0.17.4 bundle pointed every machine at the maintainer's checkout. A missing or too-old daemon is now reported by `reap daemon status`, `reap fix --check`, the agent prompt and lifecycle output instead of doing nothing. REAP and the daemon are separate packages and find each other only when they share a resolution root, so if yours do not — a global REAP with a project-local daemon, two package managers, a Node version switch — set `daemonBin` in `.reap/config.yml` or `REAP_DAEMON_BIN` for one command; `reap daemon status` shows `bin` and `binSource` so you can confirm which setting was read. `reap help` also lists `/reap.run` and `/reap.report`, which it never did.
### ko
`daemon: true` 를 쓰신다면 데몬을 한 번 설치하세요: `npm i -g @c-d-cc/reap-daemon`. v0.16 부터 문서에는 있었지만 실제로 발행된 적이 없습니다 — npm 설치본은 끊긴 심링크를 받았고, 모든 데몬 호출이 조용히 실패하는 동안 lifecycle 은 아무 일 없다는 듯 진행됐습니다. 그 뒤에 결함이 셋 더 있었습니다: 번들이 네이티브 바인딩을 인라인해 bun 에서만 기동했고, Tree-sitter 쿼리를 한 단계 위에서 찾아 인덱싱이 '성공'하고도 심볼을 하나도 뽑지 못했으며, `__dirname` 이 빌드 시점에 문자열로 박혀 **배포된 v0.17.4 번들이 모든 사용자 머신에서 메인테이너의 체크아웃을 가리켰습니다**. 데몬이 없거나 너무 낡은 경우 이제 `reap daemon status`, `reap fix --check`, agent prompt, lifecycle 출력이 그것을 알려줍니다 — 조용히 넘어가지 않습니다. REAP 와 데몬은 별개 패키지라 같은 resolution root 를 공유할 때만 서로를 찾습니다. 그렇지 않은 환경이라면(전역 REAP + 프로젝트 로컬 데몬, 서로 다른 패키지 매니저, Node 버전 전환) `.reap/config.yml` 의 `daemonBin` 이나 한 번만 쓸 `REAP_DAEMON_BIN` 을 지정하세요. `reap daemon status` 가 `bin` 과 `binSource` 를 보여주므로 어느 설정이 읽혔는지 확인할 수 있습니다. `reap help` 에 `/reap.run` 과 `/reap.report` 도 이제 나옵니다 — 여태 빠져 있었습니다.
