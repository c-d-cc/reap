...
3. **Genome 갭 분석**: goal 달성에 필요하지만 genome에 없는 정보를 식별하라
   - 에이전트가 구현 시 추론에 필요한 정보가 genome에 없다면, 그것 자체가 genome의 결함이다
   - 특히 `domain/` 디렉토리를 확인하라:
     - `domain/README.md`의 판단 기준에 따라, 이번 goal에 필요한 도메인 규칙이 domain/ 파일로 존재하는지 평가하라
     - 누락된 도메인 규칙은 mutation으로 기록하라 (target: `genome/domain/{topic}.md`)
   - 누락된 정보 각각에 대해 `.reap/life/mutations/`에 YAML 파일로 기록하라
   - mutation의 `reason`에 "genome 정보 부족: [구체적으로 무엇이 부족한지]"를 명시하라
4. goal 달성에 필요한 명세가 부족하면 보완 계획을 수립하라
5. 인간과 함께 spec을 확정하라
