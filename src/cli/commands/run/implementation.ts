...
export async function execute(paths: ReapPaths, phase?: string): Promise<void> {
  ...
  if (!phase || phase === "work") {
    verifyTransition("implementation", s, "implementation:entry");
    ...
    setTransitionNonces(s, "implementation:entry");
    await gm.save(s);
    ...
    emitOutput({
      ...
      prompt: [
        ...
        "",
        "### Discovered Tasks",
        "If new tasks are discovered during implementation that weren't in the plan:",
        "- Add them to 03-implementation.md under a 'Discovered Tasks' section with rationale.",
        "- Implement them in order, same as planned tasks.",
        "- No need to go back to planning — record the discovery and proceed.",
        "",
  ...
  if (phase === "complete") {
    verifyTransition("implementation", s, "implementation:complete");
