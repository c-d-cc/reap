export const en = {
  ...
  "entries.slug_too_long": "slug is {bytes} bytes; the limit is {limit} (Linux NAME_MAX is 255 bytes for the whole file name). Give a shorter --slug.",
  ...
  "setup.hook_failed": "codex: could not read {detail} as a valid hooks configuration, so the hook was left alone. Fix the file and run `reap setup` again.",
  "setup.done": "Open a new session in the host you just set up. Claude Code: /reap:init or /reap:evolve. Codex: ask to use reap:init or reap:evolve. REAP context appears in initialized projects.",
  "setup.done_codex": "Codex app: quit it (⌘Q) and reopen once — a running app keeps the hook list it started with, so a new conversation alone is not enough. The CLI reads the file every session. If Codex reports a hook requiring review, open /hooks in its CLI and review/trust the reap ctx --hook entry.",
  ...
  "doctor.detail.plugin_missing": "No enabled REAP plugin detected for this host (Claude Code settings.json / Codex config.toml). Run reap setup; for a development checkout, run scripts/local-install.sh",
  ...
  "doctor.kind.name_too_long": "file name too long",
  "doctor.detail.name_too_long": "{path}: {bytes} bytes (limit {limit} — Linux cannot check out a name over 255 bytes). Rename it with a shorter slug",
  ...
  "ctx.label.handoff": "Handoff: {path} — {count} section(s) ({mine})",
  "ctx.handoff.mine": "mine included",
  "ctx.handoff.none": "none of mine",
  "ctx.label.session": "This session: {key}",
