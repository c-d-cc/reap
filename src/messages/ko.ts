...
export const ko = {
  ...
  "entries.slug_too_long": "slug가 {bytes}바이트입니다. 상한은 {limit}바이트입니다(리눅스 NAME_MAX는 파일명 전체 255바이트). 더 짧은 --slug를 주세요.",
  ...
  "setup.hook_failed": "codex: {detail}을 올바른 훅 설정으로 읽지 못해 훅을 건드리지 않았습니다. 파일을 고친 뒤 `reap setup`을 다시 실행하세요.",
  "setup.done": "설치한 호스트에서 새 세션을 연다. Claude Code: /reap:init 또는 /reap:evolve. Codex: reap:init 또는 reap:evolve 스킬 사용을 요청한다. 초기화된 프로젝트에는 REAP 맥락이 주입된다.",
  "setup.done_codex": "Codex 앱은 한 번 종료했다 여세요(⌘Q) — 떠 있는 앱은 시작할 때 읽은 훅 목록을 그대로 쓰므로 새 대화만으로는 부족합니다. CLI는 세션마다 파일을 다시 읽습니다. Codex가 훅 검토를 요구하면 CLI의 /hooks에서 reap ctx --hook 항목을 검토·신뢰하세요.",
  ...
  "doctor.detail.plugin_missing": "현재 호스트에서 활성화된 REAP 플러그인을 찾지 못했다(Claude Code settings.json / Codex config.toml). reap setup을 실행한다. 개발 중인 체크아웃은 scripts/local-install.sh를 실행한다",
  ...
  "doctor.kind.name_too_long": "파일명이 너무 긺",
  "doctor.detail.name_too_long": "{path}: {bytes}바이트 (상한 {limit} — 리눅스는 255바이트를 넘는 이름을 체크아웃하지 못한다). 짧은 slug로 이름을 바꾼다",
  ...
  "ctx.label.handoff": "인계: {path} — 절 {count}개 ({mine})",
  "ctx.handoff.mine": "내 것 있음",
  "ctx.handoff.none": "내 것 없음",
  "ctx.label.session": "이 세션: {key}",
