...
export class SymbolExtractor {
  ...
  private async loadLanguage(langName: string): Promise<{ lang: any; query: any } | null> {
    ...
    try {
      ...
      // Recorded rather than printed: the CLI speaks JSON on stdout and a
      // stray line would corrupt it. `reap index status` reports the count,
      // which is what makes a missing grammar visible instead of merely quiet.
      this.loadFailures.set(langName, e instanceof Error ? e.message : String(e));
      return null;
    }
  }
  ...
  failures(): Record<string, string> {
    return Object.fromEntries(this.loadFailures);
  }
