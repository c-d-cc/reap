...
// ── Lineage goal extraction (all sources) ────────────────

export interface LineageGoal {
  id: string;
  goal: string;
}

/**
 * Read all generation goals from lineage — both uncompressed directories (meta.yml)
 * and L1 compressed .md files (frontmatter). Returns sorted by id.
 */
export async function readAllLineageGoals(lineagePath: string): Promise<LineageGoal[]> {
  let entries: string[];
  try {
    entries = await readdir(lineagePath);
  } catch {
  ...
  const goals: LineageGoal[] = [];

  for (const entry of entries.filter((e) => e.startsWith("gen-")).sort()) {
    const fullPath = join(lineagePath, entry);
    const s = await stat(fullPath).catch(() => null);
    if (!s) continue;

    if (s.isDirectory()) {
      // Uncompressed: read meta.yml
      const metaContent = await readTextFile(join(fullPath, "meta.yml"));
      if (!metaContent) continue;
      try {
        const meta = YAML.parse(metaContent) as Record<string, unknown>;
        goals.push({
          id: (meta.id as string) ?? entry,
          goal: (meta.goal as string) ?? "",
        });
      } catch { /* skip malformed */ }
    } else if (s.isFile() && entry.endsWith(".md")) {
      // L1 compressed: read frontmatter
